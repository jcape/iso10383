# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

## [0.5.1](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.5.0...iso10383-parser-v0.5.1) - 2026-09-15

### Other

- september 2026 updates

## [0.5.0](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.4.5...iso10383-parser-v0.5.0) - 2026-09-14

### Other

- august 2026 release and some inline decorations

## [0.4.5](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.4.4...iso10383-parser-v0.4.5) - 2026-02-17

### Other

- fix 0.4.4 deps

## [0.4.4](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.4.3...iso10383-parser-v0.4.4) - 2026-02-17

### Other

- Pin quick-xml to 0.39.0

## [0.4.3](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.4.2...iso10383-parser-v0.4.3) - 2026-02-10

### Added

- Add February 2026 data.

## [0.4.1](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.4.0...iso10383-parser-v0.4.1) - 2026-02-05

### Other

- enhanced clippy

## [0.4.0](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.3.3...iso10383-parser-v0.4.0) - 2026-01-15

### Added

- january 2026 data

### Other

- improve readmes, use for-the-badge style

## [0.3.3](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.3.2...iso10383-parser-v0.3.3) - 2026-01-11

### Other

- use flat-square badges

## [0.3.2](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.3.1...iso10383-parser-v0.3.2) - 2026-01-05

### Added

- serde support and roundtrip tests
- add macros and static code generation

### Other

- update READMEs and add a ChangeLog to macros
- remove license-link
- enable all-features in docsrs

## [0.3.1](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.3.0...iso10383-parser-v0.3.1) - 2025-12-21

### Added

- december data
- november data

## [0.3.0](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.2.0...iso10383-parser-v0.3.0) - 2025-10-24

### Added

- september and october data

### Other

- update documentation urls
- *(ci)* add action-validator, cargo-deny to pre-commit

## [0.2.0](https://github.com/jcape/iso10383/compare/iso10383-parser-v0.1.2...iso10383-parser-v0.2.0) - 2025-09-01

### Added

- august list
- [**breaking**] `types` crate with &mic/Mic, parser enums

### Fixed

- fix issues with iso10383-types release

### Other

- add missing enum docs, fix some style issues

## [0.1.2](https://github.com/jcape/iso10383/compare/v0.1.1...v0.1.2) - 2025-06-11

### Fixed

- *(lint)* fix parser changelog lint.

### Other

- *(test)* add june 2025 file to parser tests

## [0.1.1](https://github.com/jcape/iso10383/compare/v0.1.0...v0.1.1) - 2025-06-05

### Added

- add list object and tests of existing XML files.

### Fixed

- clippy fix for MicList

### Other

- add may data
